...
m_tot <- m_tot %>% 
  mutate(m_total = mean_rmr + mean_smr, 
         doy_id = days(date), 
...
write_rds(m_tot, here("Saved Data", 
                    "Daily_m_total.rds"))
# look at distibution ------
...
overall_parm %>%
  openxlsx::write.xlsx(here::here("results",
                                  "Total Metabolism",
                                  "gamm_m_total_param_overall.xlsx"))
ind_parm %>%
  openxlsx::write.xlsx(here::here("results",
                                  "activity Scope",
                                  "gamm_m_total_param_ind.xlsx"))

smoothers %>%
  openxlsx::write.xlsx(here::here("results",
                                  "activity Scope",
                                  "gamm_m_total_smoothers.xlsx"))
m_glance %>%
  openxlsx::write.xlsx(here::here("results",
                                  "activity Scope",
                                  "gamm_m_total_model_fit.xlsx"))
# # pridicted model
...
ggplot() + 
  ...
  geom_rect(data = rect_winter, aes(xmin = xmin,
            ...
            inherit.aes = FALSE) +
  geom_text(
    aes(x = xmin + 25, y = 191.25, label = season),
    data = rect_summer,
    size = 5, vjust = 0, hjust = 0, check_overlap = TRUE) +
  geom_text(
    aes(x = xmin + 30, y = 191.25, label = season),
    data = rect_winter,
    size = 5, vjust = 0, hjust = 0, check_overlap = TRUE) +
  geom_point(data = m_tot, aes(x = doy_id, y = m_total, 
                             colour = fish_basin), 
             size = 3, alpha = 0.5) + 
...
write_rds(p, here("Plot Objects", 
                  "total_metabolism_gamm_plot.rds"))


ggsave(plot = p, filename = here("plots",
                                 "Daily GAMM Plots", 
                                 "total_metabolism_doy_gamm.png"), width = 11,
       height = 7 )
